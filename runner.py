...
def run_test(rps, conn, output_file, args: Namespace):
    with open(output_file, "w+") as f:
        for i, requested_rps in enumerate(rps):
            ...
            print(
                f"Running test with {conn} connections and {requested_rps} RPS... ({estimated_time}m left)"
            )
...
def run_wrk_helper(args: Namespace):
    ...
    new_reports = [
        ...
        if file not in files_before and file.endswith(".txt") and os.path.isfile(file)
    ]
    return new_reports
...
def run_wrk_parser(new_reports, work_dir: str = "."):
    for report in new_reports:
        ...
        Process = Popen([exec], shell=True, stdin=PIPE, stderr=PIPE)
...
def main():
    ...
    parser.add_argument(
        "-t", type=int, default=1, help="Number of threads (default: 1)"
    )

    parser.add_argument("-d", type=int, default=15, help="Duration (default: 15)")

    parser.add_argument("-s", type=int, default=15, help="Sleep (default: 15)")
    ...
    parser.add_argument("--with-graph", action="store_true", help="Generate graph")

    parser.add_argument("url", type=str, help="Url")

    parser.add_argument(
        "-S", type=str, help="Lua script path for complicated cases loadtesting"
    )

    parser.add_argument(
        "-a", type=str, nargs="+", help="args for Lua script, eg -a foo bar"
    )
